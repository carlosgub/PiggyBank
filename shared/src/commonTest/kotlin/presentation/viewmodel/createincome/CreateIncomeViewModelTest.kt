...
class CreateIncomeViewModelTest {
    ...
    fun `Call Create but show error`() = runTest {
        createIncomeViewModel.test(this, CreateIncomeScreenState()) {
            expectInitialState()
    ...
    fun `Call Create success`() = runTest {
        val state = CreateIncomeScreenState(
            note = "note",
            dateInMillis = 1000L,
            amount = 100.0
        )
        createIncomeViewModel.test(this, state) {
            expectInitialState()
            containerHost.create()
            expectState {
                copy(
    ...
    fun `Show Error false`() = runTest {
        createIncomeViewModel.test(
            this,
            CreateIncomeScreenState(showError = true)
        ) {
            expectInitialState()

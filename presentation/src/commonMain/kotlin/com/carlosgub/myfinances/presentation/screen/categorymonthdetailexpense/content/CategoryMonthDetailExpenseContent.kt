...
package com.carlosgub.myfinances.presentation.screen.categorymonthdetailexpense.content
...
import piggybank.presentation.generated.resources.category_month_detail_data_zero_message
import piggybank.presentation.generated.resources.category_month_detail_data_zero_title

@Composable
fun CategoryMonthDetailExpenseContent(
    paddingValues: PaddingValues,
    state: CategoryMonthDetailExpenseScreenState,
...
private fun CategoryMonthDetailBody(
    ...
    if (state.showLoading || state.isInitialDataLoaded.not()) {
        ...
        Card(
            ...
            if (state.monthDetail.expenseScreenModelList.isNotEmpty()) {
                LazyColumn(
                    modifier = Modifier
                        .background(color = White)
                        .fillMaxSize()
                    ...
                    itemsIndexed(state.monthDetail.expenseScreenModelList) { index, expense ->
                        Column {
                            if (index != 0) {
                                HorizontalDivider()
                            }
...
@Composable
private fun CategoryMonthDetailHeader(state: CategoryMonthDetailExpenseScreenState) {
    Column(
